...
var $author$project$Main$Element = F5(
	function (left, top, imageSource, zIndex, fontSize) {
		return {fontSize: fontSize, imageSource: imageSource, left: left, top: top, zIndex: zIndex};
	});
var $author$project$Main$UpdatedSimpleMessageToast = function (a) {
	return {$: 'UpdatedSimpleMessageToast', a: a};
};
var $mercurymedia$elm_message_toast$MessageToast$MessageToast = F2(
	function (a, b) {
		return {$: 'MessageToast', a: a, b: b};
	});
var $mercurymedia$elm_message_toast$MessageToast$init = function (updateMsg) {
	var config = {customContainerAttributes: _List_Nil, customIconAttributes: _List_Nil, customMessageAttributes: _List_Nil, customToastAttributes: _List_Nil, delayInMs: 8000, toastsToShow: 4, updateMsg: updateMsg};
...
var $author$project$Main$init = function (_v0) {
	var model = {
		...
		leftScroll: 0,
		messageToast: $mercurymedia$elm_message_toast$MessageToast$init($author$project$Main$UpdatedSimpleMessageToast),
		topScroll: 0
	};
...
var $mercurymedia$elm_message_toast$MessageToast$getOldestToast = function (_v0) {
	var toasts = _v0.b;
	return $elm$core$List$head(toasts);
};
...
var $mercurymedia$elm_message_toast$MessageToast$popOldestToast = function (_v0) {
	var config = _v0.a;
	var toasts = _v0.b;
	var newToasts = A2(
		$elm$core$List$take,
		$elm$core$List$length(toasts) - 1,
		toasts);
	return A2($mercurymedia$elm_message_toast$MessageToast$MessageToast, config, newToasts);
};
var $mercurymedia$elm_message_toast$MessageToast$subscriptions = function (messageToast) {
	var config = messageToast.a;
	var _v0 = $mercurymedia$elm_message_toast$MessageToast$getOldestToast(messageToast);
	if (_v0.$ === 'Just') {
...
var $author$project$Main$subscriptions = function (model) {
	return $mercurymedia$elm_message_toast$MessageToast$subscriptions(model.messageToast);
};
...
var $author$project$Main$Response = F3(
	function (a, b, c) {
		return {$: 'Response', a: a, b: b, c: c};
	});
...
var $author$project$Main$elementDecoder = A6(
	...
	A2($elm$json$Json$Decode$field, 'imageSource', $elm$json$Json$Decode$string),
...
var $author$project$Main$encodeElement = function (_v0) {
	...
	var imageSource = _v0.imageSource;
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'imageSource',
				$elm$json$Json$Encode$string(imageSource)),
				_Utils_Tuple2(
				'zIndex',
				$elm$json$Json$Encode$int(zIndex)),
				_Utils_Tuple2(
				'fontSize',
				$elm$json$Json$Encode$int(fontSize))
			]));
};
...
var $mercurymedia$elm_message_toast$MessageToast$Undefined = {$: 'Undefined'};
var $mercurymedia$elm_message_toast$MessageToast$Warning = {$: 'Warning'};
...
var $mercurymedia$elm_message_toast$MessageToast$warning = function (messageToast) {
	return A2(
		$elm$core$Tuple$pair,
		{content: $mercurymedia$elm_message_toast$MessageToast$Undefined, id: 0, toastType: $mercurymedia$elm_message_toast$MessageToast$Warning},
		messageToast);
};
	...
	function (message, _v0) {
		...
		return A2(
			...
			_Utils_update(
				content,
				{
					content: $mercurymedia$elm_message_toast$MessageToast$Message(message)
				}),
			toast);
	});
		...
		switch (msg.$) {
			case 'Scroll':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{leftScroll: leftScroll, topScroll: topScroll}),
					$elm$core$Platform$Cmd$none);
			case 'UpdatedSimpleMessageToast':
				var updatedMessageToast = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{messageToast: updatedMessageToast}),
					$elm$core$Platform$Cmd$none);
			case 'NoOp':
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 'FileContentLoaded':
				...
				var _v1 = A2(
					...
					$elm$json$Json$Decode$list($author$project$Main$elementDecoder),
					fileContent);
				if (_v1.$ === 'Err') {
					var newMessageToast = A2(
						$mercurymedia$elm_message_toast$MessageToast$withMessage,
						'could not load file',
						$mercurymedia$elm_message_toast$MessageToast$warning(model.messageToast));
					return _Utils_Tuple2(
						_Utils_update(
							model,
			...
			case 'Export':
				var json = A2($elm$json$Json$Encode$list, $author$project$Main$encodeElement, model.elements);
				var command = A3(
				...
				if (_v3.$ === 'Nothing') {
					...
					var _v4 = $elm$core$String$toInt(
						A2(
							...
							'',
							$elm$core$List$head(
								$elm$core$String$words(input.query))));
					if (_v4.$ === 'Nothing') {
						if (input.query === '') {
							return _Utils_Tuple2(
								_Utils_update(
							...
							var command = $elm$http$Http$get(
								{
									expect: A2(
										...
										A2($author$project$Main$Response, input.left, input.top),
										$elm$json$Json$Decode$oneOf(
											_List_fromArray(
												[
													...
													$elm$json$Json$Decode$at,
													_List_fromArray(
														['data', '0', 'card_faces', '0', 'image_uris', 'normal']),
													$elm$json$Json$Decode$string)
												]))),
						...
						var newModel = _Utils_update(
							model,
							{
								elements: A2($elm$core$List$cons, newElement, model.elements),
								input: $elm$core$Maybe$Nothing
							});
						return _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none);
					}
			...
			case 'DoubleClick':
				...
				var top = topBeforeScroll + model.topScroll;
				var left = leftBeforeScroll + model.leftScroll;
				var newModel = function () {
					if (maybeElement.$ === 'Nothing') {
						return _Utils_update(
							...
							{
								input: $elm$core$Maybe$Just(
									{isLoading: false, left: left, query: '', top: top})
							});
					} else {
						var element = maybeElement.a;
						var newQuery = (element.fontSize > 0) ? ($elm$core$String$fromInt(element.fontSize) + (' ' + element.imageSource)) : '';
						return _Utils_update(
							model,
							{
								elements: A2(
									...
									model.elements),
								input: $elm$core$Maybe$Just(
									{isLoading: false, left: left, query: newQuery, top: top})
							});
					}
				}();
				return _Utils_Tuple2(
			...
			case 'Drag':
				...
				var lostElementList = function () {
					var _v7 = model.beingDragged;
					if (_v7.$ === 'Nothing') {
						return _List_Nil;
					} else {
						var _v8 = _v7.a;
						var lostElement = _v8.a;
						return _List_fromArray(
							[lostElement]);
					}
				}();
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							beingDragged: $elm$core$Maybe$Just(
								_Utils_Tuple3(element, left, top)),
							elements: A2(
								$elm$core$List$filter,
								$elm$core$Basics$neq(element),
								_Utils_ap(model.elements, lostElementList))
						}),
					$elm$core$Platform$Cmd$none);
			case 'DragEnd':
				var _v9 = model.beingDragged;
				if (_v9.$ === 'Nothing') {
					return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
				} else {
					var _v10 = _v9.a;
					var lostElement = _v10.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								beingDragged: $elm$core$Maybe$Nothing,
								elements: A2($elm$core$List$cons, lostElement, model.elements)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'DragOver':
			...
			default:
				...
				var leftBeforeScroll = msg.b;
				var topBeforeScroll = msg.c;
				var zIndex = $author$project$Main$nextZIndex(model.elements);
				var top = topBeforeScroll + model.topScroll;
				var left = leftBeforeScroll + model.leftScroll;
				var _v11 = model.beingDragged;
				if (_v11.$ === 'Nothing') {
					var newUrl = function () {
					...
					var topStart = _v12.c;
					var newTop = element.top + (top - topStart);
					var newLeft = element.left + (left - leftStart);
					var newModel = _Utils_update(
						model,
						{
							beingDragged: $elm$core$Maybe$Nothing,
							elements: A2(
								...
								_Utils_update(
									element,
									{left: newLeft, top: newTop, zIndex: zIndex}),
								model.elements)
						});
					return _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none);
				}
...
var $author$project$Main$InputLostFocus = {$: 'InputLostFocus'};
...
var $author$project$Main$onDropData = function (message) {
	return A2(
		$elm$html$Html$Events$on,
		'dropdata',
		A4(
	...
	function (dismissEvent, config, toast) {
		var defaultToastAttributes = _List_fromArray(
			[
				...
				'message-toast' + (' ' + $mercurymedia$elm_message_toast$MessageToast$toastTypeClass(toast.toastType))),
		...
		var updatedToastAttributes = A2($elm$core$List$append, defaultToastAttributes, config.customToastAttributes);
		return A2(
			$elm$html$Html$div,
			updatedToastAttributes,
			_List_fromArray(
				[
					A2($mercurymedia$elm_message_toast$MessageToast$viewToastIcon, config.customIconAttributes, toast),
					A2($mercurymedia$elm_message_toast$MessageToast$viewToastMessage, config.customMessageAttributes, toast),
...
var $mercurymedia$elm_message_toast$MessageToast$view = function (_v0) {
		...
		return config.updateMsg(
			A2(
				...
				A2(
					$elm$core$List$filter,
					function (toast) {
						return !_Utils_eq(toast.id, toastMessage.id);
					},
					toasts)));
	};
	...
	if (!_v1.b) {
		...
		var updatedContainerAttributes = A2($elm$core$List$append, defaultContainerAttributes, config.customContainerAttributes);
		return A2(
			$elm$html$Html$div,
...
var $author$project$Main$viewElement = function (element) {
	...
	var commonAttributes = _List_fromArray(
		[
			...
			$author$project$Main$onDoubleClick(
			...
			$author$project$Main$Drag(element)),
	...
	if (fontSize > 0) {
		...
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$text(imageSource)
				]));
	} else {
		return A2(
			$elm$html$Html$img,
			_Utils_ap(
				_List_fromArray(
					[
						$elm$html$Html$Attributes$src(imageSource),
...
var $author$project$Main$view = function (model) {
	var messageToast = _List_fromArray(
		[
			$mercurymedia$elm_message_toast$MessageToast$view(model.messageToast)
		]);
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$id('activeInput'),
									$elm$html$Html$Events$onBlur($author$project$Main$InputLostFocus)
								]),
							_List_Nil)
						]))
				]);
	...
	var importExportButtons = _List_fromArray(
		[
			A2(
			$elm$html$Html$div,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Events$onClick($author$project$Main$Import)
						]),
					_List_fromArray(
						[
	...
	return A2(
		...
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'position', 'absolute'),
				A2($elm$html$Html$Attributes$style, 'width', '50000px'),
				A2($elm$html$Html$Attributes$style, 'height', '50000px'),
		...
		_Utils_ap(
			...
			_Utils_ap(
				...
				_Utils_ap(
					maybeInput,
					A2($elm$core$List$map, $author$project$Main$viewElement, model.elements)))));
};
